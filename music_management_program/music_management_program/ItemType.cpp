...
void ItemType::Play()
{
	if (f_Type == 1)
	{
		...
		if (ShellExecute(NULL, NULL, "C://Program Files (x86)/Google/Chrome/Application/chrome.exe", dir, NULL, SW_HIDE) < HINSTANCE(32))
			PlaySound(TEXT(dir), NULL, SND_FILENAME);
	}
	else if (f_Type == 2)
	{
		char address[55] = "explorer ";
		strcat_s(address, f_Name.c_str());
		if (ShellExecute(NULL, NULL, "C://Program Files (x86)/Google/Chrome/Application/chrome.exe", f_Name.c_str(), NULL, SW_HIDE) < HINSTANCE(32))
			system(address);
	}
	else
		return;
