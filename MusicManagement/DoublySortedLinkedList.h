...
struct DoublyNodeType
{
	...
	///**
	//*	@brief	Copy List.
	//*	@pre	node should be initialized.
	//*	@post	copy node.
	//*	@return DoublyNodeType<T>	node same with data.
	//*/
	//DoublyNodeType<T>& operator=(const DoublyNodeType<T>& data)
	//{
	//	this->data = data.data;
	//	this->prev = data.prev;
	//	this->next = data.next;
...
public:
	...
	*/
	...
	/**
	*	Call data in list.
	*	@brief	call data in list by using iterator class operation next for input time.
	*	@pre	list should be initialized.
	*	@post	call data in index order.
	*	@param	index	insert order in list.
	*	@return return data of Node in index node in list.
	*/
	...
	*	@pre	list should be initialized.
	*	@post	copy list.
	*	@return DoublySortedLinkedList<T>	list same with data.
	*/
	DoublySortedLinkedList<T>& operator=(const DoublySortedLinkedList<T>& data);
...
//리스트에 있는 모든 레코드를 출력한다.
template <typename T>
void DoublySortedLinkedList<T>::DisplayItem()
{
	for (int i = m_nLength - 1; i >= 0; i--)
		cout << (*this)[i];
}

//index 번째로 삽입된 노드의 데이터를 리턴한다.
template <typename T>
T& DoublySortedLinkedList<T>::operator[](int index)
{
	DoublyIterator<T> itor(*this);
	for (int i = 0; i <= index % m_nLength; i++)
		itor.Next();
...
//Index Operator.
template <typename T>
T& DoublySortedLinkedList<T>::operator[](int index) const
{
	DoublyIterator<T> itor(*this);
	for (int i = 0; i <= index % m_nLength; i++)
		itor.Next();
...
//Copy operator.
template<typename T>
DoublySortedLinkedList<T>& DoublySortedLinkedList<T>::operator=(const DoublySortedLinkedList<T>& data)
{
	if (data.m_nLength==0)
	{
		m_pFirst = new DoublyNodeType<T>;
	...
	for (int i = 0; i < data.m_nLength; i++)
		Add(data[i]);
	return *this;
}
